...
const el = {
  ...
  categoriesShareBtn: document.getElementById("categories-share-btn"),
  looseImagesSection: document.getElementById("loose-images-section"),
  looseImagesTitle: document.getElementById("loose-images-title"),
  looseImagesGrid: document.getElementById("loose-images-grid"),
  categoriesNote: document.getElementById("categories-note"),
  categoriesNoteText: document.getElementById("categories-note-text"),
  categoriesNoteToggle: document.getElementById("categories-note-toggle"),

  breadcrumbDetail: document.getElementById("breadcrumb-detail"),
  backBtn: document.getElementById("back-btn"),
  detailNote: document.getElementById("detail-note"),
  detailNoteText: document.getElementById("detail-note-text"),
  detailNoteToggle: document.getElementById("detail-note-toggle"),
  ...
  lightboxDonate: document.getElementById("lightbox-donate"),
  lightboxNameText: document.getElementById("lightbox-name-text"),
  lightboxPhotoTitle: document.getElementById("lightbox-photo-title"),
  lightboxKeywordsCell: document.getElementById("lightbox-keywords-cell"),
  lightboxKeywordsText: document.getElementById("lightbox-keywords-text"),
...
// Zählt anonym ein Ereignis hoch (Besuch, Bild angesehen) — läuft im
// Hintergrund, verzögert oder blockiert die Seite nie und wird bei Fehlern
// einfach stillschweigend ignoriert.
function trackEvent(type) {
  try {
    fetch(`/api/track?type=${type}`, { keepalive: true }).catch(() => {});
  } catch {
    // Ignorieren.
  }
}
...
window.addEventListener("load", () => {
  ...
  el.categoriesShareBtn.addEventListener("click", shareCurrentCategory);
  el.detailShareBtn.addEventListener("click", shareCurrentCategory);
  ...
  el.lightboxDonate.href =
    "https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=oe%40oem.de" +
    "&currency_code=EUR&item_name=" +
    encodeURIComponent("Spende für die Fotogalerie");
...
async function init() {
  ...
  const sharedId = params.get("bild");

  if (categoryId) {
    await openSharedCategory(categoryId);
  } else {
    await loadFolder([]);
  }
...
// Öffnet eine über Link geteilte Kategorie direkt (mit vollständigem Pfad).
async function openSharedCategory(id) {
  try {
    const res = await fetch(`/api/path?id=${id}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || "Kategorie-Link ungültig.");
    if (data.hasSubcategories) await loadFolder(data.path);
    else await loadLeaf(data.path);
  } catch (e) {
    showToast("Kategorie-Link konnte nicht geöffnet werden.");
    await loadFolder([]);
  }
}

// Kopiert einen Direktlink zur aktuell angezeigten Kategorie.
function shareCurrentCategory() {
  if (currentPath.length === 0) return;
  const folderId = currentPath[currentPath.length - 1].id;
  const url = `${window.location.origin}/teilen/kategorie/${folderId}`;
...
// Hält die Adressleiste synchron zum aktuellen Kategorie-Pfad, damit auch
// einfaches Kopieren der Adresse funktioniert.
function updateCategoryUrl(path) {
  const url = new URL(window.location.href);
  if (path.length > 0) {
    url.searchParams.set("kategorie", path[path.length - 1].id);
  } else {
...
// -------------------- Orts-Beschreibung (beschreibung.txt) --------------------

// Zeigt eine optionale Orts-Beschreibung an: standardmäßig auf 3 Zeilen
// begrenzt, mit "Mehr lesen"-Umschalter, falls der Text länger ist.
function renderFolderNote(containerEl, textEl, toggleBtn, noteText) {
  ...
  // Nach dem Rendern prüfen, ob der Text die 3 Zeilen wirklich überschreitet
  // — nur dann den Umschalter überhaupt anzeigen.
  requestAnimationFrame(() => {
    const isOverflowing = textEl.scrollHeight > textEl.clientHeight + 2;
    toggleBtn.classList.toggle("hidden", !isOverflowing);
  });

  toggleBtn.onclick = () => {
    const isExpanded = textEl.classList.toggle("clamped") === false;
    toggleBtn.textContent = isExpanded ? "Weniger anzeigen" : "Mehr lesen";
  };
}
...
async function loadFolder(path) {
  ...
  try {
    ...
    const looseImages = data.looseImages || [];

    if (categories.length === 0 && looseImages.length === 0) {
      showError(
        path.length === 0
    ...
    el.categoriesShareBtn.classList.toggle("hidden", path.length === 0);
    updateCategoryUrl(path);
    renderFolderNote(el.categoriesNote, el.categoriesNoteText, el.categoriesNoteToggle, data.folderNote);
    ...
    // Lose Bilder, die direkt in diesem Ordner liegen (nicht in einer
    // Unterkategorie einsortiert), unterhalb der Kategorie-Kacheln zeigen.
    el.looseImagesGrid.innerHTML = "";
    if (looseImages.length > 0) {
      el.looseImagesTitle.textContent =
        path.length === 0
          ? "Direkt hier abgelegt, ohne Kategorie"
          : `Direkt in „${path[path.length - 1].name}“, ohne Unterkategorie`;
      const label = path.length ? path[path.length - 1].name : "Ohne Kategorie";
      looseImages.forEach((file, index) => {
        el.looseImagesGrid.appendChild(buildThumb(file, index, looseImages, label));
      });
      el.looseImagesSection.classList.remove("hidden");
    } else {
      el.looseImagesSection.classList.add("hidden");
    }
  } catch (e) {
...
async function loadLeaf(path) {
  ...
  try {
    ...
    updateCategoryUrl(path);
    renderFolderNote(el.detailNote, el.detailNoteText, el.detailNoteToggle, data.folderNote);
...
function buildThumb(file, index, imagesArray, labelForAlt) {
  ...
  const displayLabel = file.photoTitle || labelForAlt;
  thumb.setAttribute(
    "aria-label",
    `${displayLabel}, Bild ${index + 1} von ${imagesArray.length} vergrößern`
  );
  const open = () => {
  ...
  const img = document.createElement("img");
  img.alt = `${displayLabel}, Bild ${index + 1} von ${imagesArray.length}`;
  img.loading = "lazy";
...
function shareCurrentImage() {
  const file = currentImages[currentLightboxIndex];
  if (!file) return;
  const url = `${window.location.origin}/teilen/bild/${file.id}`;
...
function openLightbox(index) {
  ...
  el.lightboxImg.alt = `${file.photoTitle || currentImagesLabel}, Bild ${index + 1} von ${currentImages.length}`;
  el.lightboxCaption.textContent = `${index + 1} / ${currentImages.length}`;
  el.lightboxNameText.textContent = file.name;

  updatePhotoTitleAndKeywords(file);
  updateNoteCell(file);
...
// Zeigt Titel + Schlagworte an, falls (schon) vorhanden — bleibt sonst
// ausgeblendet, bis loadPhotoMeta() sie nachgeladen hat.
function updatePhotoTitleAndKeywords(file) {
  if (file.photoTitle) {
    el.lightboxPhotoTitle.textContent = file.photoTitle;
    el.lightboxPhotoTitle.classList.remove("hidden");
  } else {
    el.lightboxPhotoTitle.classList.add("hidden");
  }

  if (file.photoKeywords && file.photoKeywords.length) {
    el.lightboxKeywordsText.textContent = file.photoKeywords.join(", ");
    el.lightboxKeywordsCell.classList.remove("hidden");
  } else {
    el.lightboxKeywordsCell.classList.add("hidden");
  }
}

// Bemerkung: bevorzugt die Lightroom-Bildunterschrift (falls schon
// nachgeladen), sonst die schnell verfügbare Drive-eigene Beschreibung.
function updateNoteCell(file) {
  const noteText = (file.photoCaption || file.description || "").trim();
  if (noteText) {
    el.lightboxNoteText.textContent = noteText;
...
// Holt Titel/Bildunterschrift/Schlagworte erst jetzt, bei tatsächlichem
// Öffnen dieses einen Bildes — nicht beim Laden der ganzen Kategorie.
// Wurden sie für dieses Bild schon einmal geladen (z.B. erneutes Öffnen),
// wird kein zweites Mal nachgefragt.
async function loadPhotoMeta(index, file) {
  if (file._photoMetaLoaded) return;
  const requestId = ++photoMetaRequestId;

  try {
    const res = await fetch(`/api/photometa?id=${file.id}`);
    if (!res.ok) return;
    const meta = await res.json();
    if (requestId !== photoMetaRequestId) return; // inzwischen weitergeklickt

    file._photoMetaLoaded = true;
    file.photoTitle = meta.title || null;
    file.photoCaption = meta.caption || null;
    file.photoKeywords = meta.keywords || null;

    updatePhotoTitleAndKeywords(file);
    updateNoteCell(file);
    el.lightboxImg.alt = `${file.photoTitle || currentImagesLabel}, Bild ${index + 1} von ${currentImages.length}`;
  } catch {
    // Stillschweigend ignorieren — Grundinfos sind bereits sichtbar.
  }
}
