...
<meta name="twitter:image" content="${escapeHtml(image)}">`
  ...
  const visibleImage = image
    ? `<img src="${escapeHtml(image)}" alt="${escapeHtml(title)}" style="max-width:100%;height:auto;border-radius:4px;">`
    : "";
...
<style>
  body { font-family: sans-serif; background: #111; color: #f2ede4; max-width: 720px; margin: 40px auto; padding: 0 20px; }
  a { color: #c9924f; }
</style>
</head>
<body>
  ...
  <p><a href="${escapeHtml(redirectUrl)}">In der vollständigen Galerie ansehen →</a></p>
...
module.exports = async (req, res) => {
  ...
  if (!id || (type !== "bild" && type !== "kategorie")) {
    res.status(400);
    res.end("Ungültiger Teilen-Link.");
    return;
  }
