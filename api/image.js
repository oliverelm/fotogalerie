...
// Enthält auch Titel/Bildunterschrift/Schlagworte aus den Foto-eigenen
// IPTC/XMP-Metadaten, damit die Anzeige identisch zur normalen Kategorie-
// Ansicht ist.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const images = await Promise.all(
      files.map(async (f) => {
        const photoMeta = await getPhotoMeta(f.id);
        return {
          ...f,
          country: countryFromLocation(f.imageMediaMetadata && f.imageMediaMetadata.location),
          description: photoMeta.caption || f.description || null,
          photoTitle: photoMeta.title,
          photoKeywords: photoMeta.keywords,
        };
      })
    );
